...
Weight sparsity modification: Gradual Magnitude Pruning with Gradient-Guided
Regrowth (simplified RigL). The network maintains a fixed sparsity budget but
the topology adapts — dead weights are recycled to where gradients are largest.
"""
...
class SparseTopologyManager:
    ...
    def __init__(self, model, target_sparsity: float = 0.3,
        ...
        # Collect 2D weight matrices (skip embeddings, LayerNorm, biases)
        self.params = []
        self.masks = {}
        for name, param in model.named_parameters():
            if param.ndim == 2 and 'token_emb' not in name and 'head.' not in name:
...
def main():
    ...
    parser.add_argument("--weight_sparsity", type=float, default=0.3,
                        help="Target weight sparsity (0 to disable, default: 0.3)")
    parser.add_argument("--sparsity_warmup", type=int, default=1000,
                        help="Steps to ramp sparsity from 0 to target (default: 1000)")
    parser.add_argument("--prune_interval", type=int, default=100,
                        help="Steps between prune-regrow cycles (default: 100)")
    parser.add_argument("--drop_fraction", type=float, default=0.3,
                        help="Fraction of active weights to prune/regrow each cycle (default: 0.3)")
    ...
    # Sparse topology manager (RigL-style prune-regrow)
    sparse_manager = None
    if args.weight_sparsity > 0:
        ...
        if is_main(use_ddp):
            n_managed = sum(p.numel() for _, p in sparse_manager.params)
            print(f"  Weight sparsity: target={args.weight_sparsity:.0%}, "
                  f"warmup={args.sparsity_warmup} steps, "
                  f"prune_interval={args.prune_interval}, "
                  f"managed params={n_managed:,}")
...
def _run_post_training_eval(args, wandb_url: str = ""):
    ...
    if not modification:
        ...
        if getattr(args, 'weight_sparsity', 0) > 0:
            parts.append(f"Weight sparsity {args.weight_sparsity:.0%} (RigL-style prune-regrow)")
